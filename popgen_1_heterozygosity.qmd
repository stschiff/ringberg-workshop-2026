...
text(1.8, 0.5, labels = sprintf("%.2f - %.2f Mya\n(90%% probability interval", quantile_boundaries[1], quantile_boundaries[2]), cex = 0.8)
```

:::

5. Let visualise effective population size in space, for example using the `maps` package.
...
# Create a color palette based on heterozygosity values
col_palette <- colorRampPalette(c("blue", "red"))(100)
ne_colors <- col_palette[cut(dat$Ne, breaks = 100, labels = FALSE)]

points(x = dat$longitude, y = dat$latitude, pch = 20, col = ne_colors)

# Add a legend
legend("bottomleft", 
       legend = c(sprintf("%.0f", min(dat$Ne, na.rm = TRUE)),
                  sprintf("%.0f", max(dat$Ne, na.rm = TRUE))),
       col = c(col_palette[1], col_palette[100]),
       pch = 20,
...
6.  Plot heterozygosity as a function of the distance from Africa, for example from Addis Abeba in East Africa (Latitude ~ 39, Longitude ~9). You can compute the distance using the Haversine formula:
